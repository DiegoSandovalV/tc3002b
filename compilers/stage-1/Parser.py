...
class Parser:
    ...
    def __init__(self, filepath):
        ...
        self.firstExtendedMultiplicativeExpression = set(
            (ord('*'), ord('/'), Tag.MOD))

        self.firstMultiplicativeExpression = self.firstUnaryExpression

        self.firstExtendedAdditiveExpression = set(
            (ord('+'), ord('-')))
        ...
        self.firstExtendedEqualityExpression = set(
            (ord('='), Tag.NEQ))

        self.firstEqualityExpression = self.firstRelationalExpression

        self.firstExtendedConditionalExpression = set(
            (Tag.OR,))
        ...
        self.firstMovementStatement = set(
            (Tag.FORWARD, Tag.BACKWARD, Tag.LEFT,
             Tag.RIGHT, Tag.SETXY, Tag.SETX, Tag.SETY, Tag.HOME))
        ...
        self.firstStructuredStatement = self.firstRepetitiveStatement.union(
            self.firstConditionalStatement)

        self.firstSimpleStatement = (
            self.firstMovementStatement
            .union(self.firstAssigmentStatement)
            .union(self.firstDrawingStatement)
            .union(self.firstTextStatement)
            .union(self.firstDeclarationStatement)
        )
        ...
        self.firstProgram = self.firstDeclarationStatement.union(
            self.firstStatementSequence)
    ...
    def primaryExpression(self):
        if self.token.tag in self.firstPrimaryExpression:
            ...
            self.error("expected a primary expression before " +
                       str(self.token))
    ...
    def unaryExpression(self):
        if self.token.tag in self.firstUnaryExpression:
            ...
            self.error("expected an unary expression before " +
                       str(self.token))
    ...
    def additiveExpression(self):
        if self.token.tag in self.firstAdditiveExpression:
            ...
            self.error("expected an additive expression before " +
                       str(self.token))
    ...
    def equalityExpression(self):
        if self.token.tag in self.firstEqualityExpression:
            ...
            self.error("expected an equality expression before " +
                       str(self.token))
    ...
    def conditionalExpression(self):
        if self.token.tag in self.firstConditionalExpression:
            ...
            self.error("expected an conditional expression before " +
                       str(self.token))
    ...
    def expression(self):
        if self.token.tag in self.firstExpression:
            ...
            self.error("expected an expression before " +
                       str(self.token))
    ...
    def ifElseStatement(self):
        if self.token.tag == Tag.IFELSE:
            ...
            self.error("expected an IFELSE expression before " +
                       str(self.token))
    ...
    def conditionalStatement(self):
        if self.token.tag in self.firstConditionalStatement:
            ...
            self.error(
                "expected an conditional expression before " + str(self.token))
    ...
    def repetitiveStatement(self):
        if self.token.tag == Tag.WHILE:
            ...
            self.error(
                "expected an repetitive expression before " + str(self.token))
    ...
    def structuredStatement(self):
        if self.token.tag in self.firstStructuredStatement:
            ...
            self.error(
                "expected an structured expression before " + str(self.token))
    ...
    def element(self):
        if self.token.tag in self.firstElement:
            ...
            self.error("expected an element expression before " +
                       str(self.token))
    ...
    def penWidthStatement(self):
        if self.token.tag == Tag.PENWIDTH:
            ...
            self.error("expected a PENWIDTH statement before " +
                       str(self.token))
    ...
    def penDownStatement(self):
        if self.token.tag == Tag.PENDOWN:
            ...
            self.error("expected a PENDOWN statement before " +
                       str(self.token))
    ...
    def penUpStatement(self):
        if self.token.tag == Tag.PENUP:
            ...
            self.error("expected a PENUP statement before " +
                       str(self.token))
    ...
    def drawingStatement(self):
        if self.token.tag in self.firstDrawingStatement:
            ...
            self.error("expected a drawing statement before " +
                       str(self.token))
    ...
    def setXYStatement(self):
        if self.token.tag == Tag.SETXY:
            ...
            self.error("expected a SETXY statement before " +
                       str(self.token))
    ...
    def setXStatement(self):
        if self.token.tag == Tag.SETX:
            ...
            self.error("expected a SETX statement before " +
                       str(self.token))
    ...
    def setYStatement(self):
        if self.token.tag == Tag.SETY:
            ...
            self.error("expected a SETY statement before " +
                       str(self.token))
    ...
    def leftStatement(self):
        if self.token.tag == Tag.LEFT:
            ...
            self.expression()
            self.check(ord(')'))
        elif self.token.tag == Tag.LT:
            self.check(Tag.LT)
            self.check(ord('('))
            self.expression()
            self.check(ord(')'))
        else:
            self.error("expected a LEFT statement before " +
                       str(self.token))
    ...
    def rightStatement(self):
        if self.token.tag == Tag.RIGHT:
            ...
            self.error("expected a RIGHT statement before " +
                       str(self.token))
    ...
    def backwardStatement(self):
        if self.token.tag == Tag.BACKWARD:
            ...
            self.error("expected a BACKWARD statement before " +
                       str(self.token))
    ...
    def forwardStatement(self):
        if self.token.tag == Tag.FORWARD:
            ...
            self.error("expected a FORWARD statement before " +
                       str(self.token))

    def homeStatement(self):
        if self.token.tag == Tag.HOME:
            self.check(Tag.HOME)
            self.check(ord('('))
            self.check(ord(')'))
        else:
            self.error("expected a HOME statement before " +
                       str(self.token))
    ...
    def movementStatement(self):
        if self.token.tag in self.firstMovementStatement:
            ...
            self.error("expected a movement statement before " +
                       str(self.token))
    ...
    def assigmentStatement(self):
        if self.token.tag == Tag.ID:
            ...
            self.error("expected an ASSIGMENT statement before " +
                       str(self.token))
    ...
    def declarationStatement(self):
        if self.token.tag == Tag.VAR:
            ...
            self.error(
                "expected a DECLARATION statement before " + str(self.token))
